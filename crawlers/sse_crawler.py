...
def _is_confirmed_status(status: str) -> bool:
    return status in {"data_found", "state_confirmed_empty"}


def _wait_sections_confirmed(
    page: Page,
    reference_date: str,
    timeout_ms: int = CONTENT_REFRESH_TIMEOUT_MS,
) -> tuple[list[dict[str, str]], str, list[dict[str, str]], str]:
    deadline = dt.datetime.now().timestamp() + timeout_ms / 1000.0
    last_drafting_items: list[dict[str, str]] = []
    last_drafting_status = "xpath_missing"
    last_inquiry_items: list[dict[str, str]] = []
    last_inquiry_status = "xpath_missing"

    while dt.datetime.now().timestamp() < deadline:
        drafting_spans = page.locator(f"xpath={DRAFTING_XPATH}")
        last_drafting_items, last_drafting_status = _extract_items_with_status(
            drafting_spans,
            reference_date=reference_date,
            market="SH",
        )
        ...
        last_inquiry_items, last_inquiry_status = _extract_items_with_status(
            inquiry_spans,
            reference_date=reference_date,
            market="SH",
        )

        if _is_confirmed_status(last_drafting_status) and _is_confirmed_status(last_inquiry_status):
            return last_drafting_items, last_drafting_status, last_inquiry_items, last_inquiry_status
...
def fetch(
    ...
    with sync_playwright() as p:
        ...
        for target_date in date_list:
            ...
            drafting_items, drafting_status, inquiry_items, inquiry_status = _wait_sections_confirmed(
                page,
                reference_date=reference_date,
                timeout_ms=CONTENT_REFRESH_TIMEOUT_MS,
            )
