...
def _extract_items_from_rows(
    ...
    if saw_cell and nbsp_count > 0:
        return [], "state_confirmed_empty", nbsp_count
    if saw_cell:
        return [], "state_pending", nbsp_count
...
def _wait_rows_confirmed(
    ...
    last_status = "xpath_missing"
    last_nbsp = 0

    while time.monotonic() < deadline:
        last_items, last_status, last_nbsp = _extract_items_from_rows(
            page=page,
            rows_xpath=rows_xpath,
            reference_date=reference_date,
            market=market,
        )
        if _is_confirmed_status(last_status):
            return last_items, last_status, last_nbsp
        page.wait_for_timeout(CALENDAR_SYNC_POLL_MS)
...
def _capture_and_log_for_date(
    ...
    drafting_items, drafting_status, drafting_nbsp = _wait_rows_confirmed(
        page=page,
    ...
    inquiry_items, inquiry_status, inquiry_nbsp = _wait_rows_confirmed(
        page=page,
